...
class ModelConverter:
    ...
    def product_template_to_inventory_item(self, env: OdooEnvWrapper, prod_tmpl):
        """
    ...
    def product_to_inventory_item(self, env: OdooEnvWrapper, prod):
        """
    ...
    def product_to_related_data(self, env: OdooEnvWrapper, prod):
        """
    ...
    def product_to_unit_of_measure(self, env: OdooEnvWrapper, prod):
        """
    ...
    def product_template_to_related_data(self, env: OdooEnvWrapper, prod_tmpl):
        """
    ...
    def product_template_to_unit_of_measure(self, env: OdooEnvWrapper, prod_tmpl):
        """
    ...
    def stock_picking_to_doc_description(self, env: OdooEnvWrapper, pick, document_type_name):
        """
        ...
        if pick:
            ...
            vals.update({
                'id': CommonUtils.encode_stock_picking_id(pick.id),
                ...
                'scanLocations': self._stock_picking_get_scan_locations(env, pick),
                'scanInUnits': False,
            ...
            if document_type_name.lower() in ['receiving', 'allocation']:
                # noinspection SpellCheckingInspection
                vals.update({'scanSNOnlyOnPutaway': env.w15_settings.scan_serials_on_allocation})

        return self._clear_output_dict(vals)

    def _stock_picking_get_scan_locations(self, env: OdooEnvWrapper, pick):
        """
        True if we need to scan locations on mobile device
        """
        if not env.storage_locations_enabled:
            return False
    ...
    def stock_picking_to_actual_lines(self, env: OdooEnvWrapper, pick, ignore_zero_done: bool):
        """
        ...
        for line in pick.move_line_ids_without_package:
            if ignore_zero_done and self._get_actual_quantity(env, line) <= 0:
                continue
            ...
            adding_line = {
                ...
                'actualQuantity': self.clear_to_str(self._get_actual_quantity(env, line)),
    ...
    def stock_picking_to_expected_lines(self, env: OdooEnvWrapper, pick):
        """
    ...
    def _get_actual_quantity(self, env: OdooEnvWrapper, odoo_line):
    ...
    def _get_source_doc_type(cls, clv_doc_type: str, odoo_warehouse) -> str:
        ...
        if lowered_doc_type_name in cls._SOURCE_DOC_TYPE_MAPPING:
            ...
            delivery_steps = odoo_warehouse.delivery_steps.lower()
            ...
            elif delivery_steps in cls._SOURCE_DOC_TYPE_MAPPING[lowered_doc_type_name]:
                return cls._SOURCE_DOC_TYPE_MAPPING[lowered_doc_type_name][delivery_steps]
    ...
    def _get_result_doc_type(cls, clv_doc_type: str, odoo_warehouse) -> str:
        ...
        if lowered_doc_type_name in cls._RESULT_DOC_TYPE_MAPPING:
            ...
            elif delivery_steps in cls._RESULT_DOC_TYPE_MAPPING[lowered_doc_type_name]:
                return cls._RESULT_DOC_TYPE_MAPPING[lowered_doc_type_name][delivery_steps]
