...
class TableContactsProcessor(TableProcessorBase):
    ...
    _mapping_fields : List[FieldInfo] = [
        ...
        FieldInfo(api_name_arg='countryCode'.lower(), api_type_arg=str, odoo_name_arg='country_code', odoo_type_arg=str),
        ...
        FieldInfo(api_name_arg='isCompany'.lower(), api_type_arg=bool, odoo_name_arg='is_company', odoo_type_arg=bool),
        FieldInfo(api_name_arg='addressType'.lower(), api_type_arg=str, odoo_name_arg='type', odoo_type_arg=str)
    ]

    def __init__(self):
        self._api_to_odoo_map = FieldInfo.create_api_to_odoo_field_map(self._mapping_fields)

    def _get_rows_int(self, env: OdooEnvWrapper, query, device_info, offset, limit, request_count: bool) -> List:
        ...
        if request_count:
            contacts_count = env.partners.search_count(domain_filter)
            return [contacts_count, None]

        partners = env.partners.search(domain_filter, limit=limit, offset=offset, order='id ASC')
